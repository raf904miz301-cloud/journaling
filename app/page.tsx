  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        ...
        <h2 style={{ fontFamily: "'Shippori Mincho', serif", fontSize: '1.2rem', color: '#2c2520', marginBottom: 12 }}>今日はここまで</h2>
        <p style={{ fontSize: '0.875rem', color: '#5a4f4a', lineHeight: 1.9, marginBottom: 8 }}>1日3回まで利用できます。</p>
    ...
    <>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        body { font-family:'Zen Maru Gothic',sans-serif; background-color:#faf7f2; color:#2c2520; min-height:100vh;
          background-image: radial-gradient(ellipse at 20% 50%,rgba(212,132,122,0.06) 0%,transparent 60%), radial-gradient(ellipse at 80% 20%,rgba(139,168,136,0.06) 0%,transparent 50%); background-attachment:fixed; }
        textarea,input { font-family:'Zen Maru Gothic',sans-serif; }
        textarea:focus,input:focus { outline:none; }
        ::-webkit-scrollbar { width:6px; } ::-webkit-scrollbar-thumb { background:#e8d5b0; border-radius:3px; }
      `}</style>
      ...
      <div style={{ maxWidth: 660, margin: '0 auto', padding: '0 20px', minHeight: '100vh' }}>
        ...
        {step === 1 && (
          <div style={{ background:'#fffef9', borderRadius:24, padding:'36px', boxShadow:'0 4px 32px rgba(44,37,32,0.07)', border:'1px solid rgba(255,255,255,0.8)', marginBottom:20, animation:'cardIn 0.5s ease forwards', position:'relative', overflow:'hidden' }}>
              ...
              {feelings.map((f,i) => (
                <div key={i} style={{ display:'flex', alignItems:'center', gap:12 }}>
                  ...
                  <input type="text" value={f}
                    ...
                    style={{ flex:1, background:'#faf7f2', border:'1.5px solid #e8d5b0', borderRadius:12, padding:'12px 16px', fontSize:'0.9rem', color:'#2c2520', transition:'all 0.3s ease' }}
            ...
            <button onClick={handleStep1Submit} disabled={feelings.filter(f=>f.trim()).length<3||loading||remaining<=0}
              style={{ width:'100%', background:feelings.filter(f=>f.trim()).length>=3&&remaining>0?'linear-gradient(135deg,#d4847a,#b85c52)':'#e8d5b0', color:'white', border:'none', borderRadius:16, padding:'16px 32px', fontFamily:"'Zen Maru Gothic',sans-serif", fontSize:'0.95rem', cursor:feelings.filter(f=>f.trim()).length>=3&&remaining>0?'pointer':'not-allowed', transition:'all 0.3s ease', boxShadow:feelings.filter(f=>f.trim()).length>=3&&remaining>0?'0 4px 20px rgba(184,92,82,0.3)':'none' }}>
        ...
        {([2,3,4,5] as const).map(s => {
          ...
          return (
            <div key={s} style={{ background:'#fffef9', borderRadius:24, padding:'36px', boxShadow:'0 4px 32px rgba(44,37,32,0.07)', border:'1px solid rgba(255,255,255,0.8)', marginBottom:20, animation:'cardIn 0.5s ease forwards', position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', top:0, left:0, right:0, height:3, background:'linear-gradient(90deg,#d4847a,#c9a96e,#8ba888)', opacity:0.6 }} />
              ...
              <textarea value={data.freeText} onChange={e=>updateStepField(s,'freeText',e.target.value)} placeholder="自由に書いてみてください…" rows={3}
              ...
              />
              {loading?<LoadingDots />:(
                <button onClick={()=>handleStepSubmit(s)} disabled={!data.selectedSuggestion&&!data.freeText.trim()}
                  style={{ width:'100%', background:(data.selectedSuggestion||data.freeText.trim())?'linear-gradient(135deg,#d4847a,#b85c52)':'#e8d5b0', color:'white', border:'none', borderRadius:16, padding:'16px 32px', fontFamily:"'Zen Maru Gothic',sans-serif", fontSize:'0.95rem', cursor:(data.selectedSuggestion||data.freeText.trim())?'pointer':'not-allowed', transition:'all 0.3s ease', boxShadow:(data.selectedSuggestion||data.freeText.trim())?'0 4px 20px rgba(184,92,82,0.3)':'none' }}>
                  {s===5?'まとめを見る ✨':'次へ進む →'}
                </button>
              )}
            </div>
        ...
        {loading&&step!==1&&!stepData[step]?.aiResponse&&(
          <div style={{ background:'#fffef9', borderRadius:24, padding:'40px 36px', textAlign:'center', boxShadow:'0 4px 32px rgba(44,37,32,0.07)', marginBottom:20, animation:'cardIn 0.5s ease forwards' }}>
            <p style={{ fontSize:'0.85rem', color:'#8a7e79', marginBottom:16 }}>気持ちに寄り添っています…</p>
            <LoadingDots />
          </div>
        )}
