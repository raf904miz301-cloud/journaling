...
const MAX_DAILY = 3;
const INSTAGRAM_ID = 'studio_mizuki'; // ← InstagramIDをここに入力
...
function SuggestionBtn({
  ...
  return (
    <button onClick={onClick} style={{
      background: selected
        ? 'linear-gradient(135deg, #f5e6e0, #fff0eb)'
        : '#fffef9',
  ...
  return (
    <div onClick={onClose} style={{
      ...
      <div onClick={e => e.stopPropagation()} style={{
        background: '#fffef9', borderRadius: 28,
        ...
        <h2 style={{
          ...
          fontSize: '1.2rem', color: '#2c2520',
          marginBottom: 12, letterSpacing: '0.08em',
        }}>今日はここまで</h2>
        <p style={{ fontSize: '0.875rem', color: '#5a4f4a', lineHeight: 1.9, marginBottom: 8 }}>
          1日3回まで利用できます。
        </p>
        <p style={{ fontSize: '0.875rem', color: '#5a4f4a', lineHeight: 1.9, marginBottom: 28 }}>
          毎朝0時にリセットされます 🌅<br />
          もっと話したいときは、インスタからDMを送ってください。
        </p>
        <a
        ...
        <button onClick={onClose} style={{
          ...
          fontSize: '0.8rem', color: '#8a7e79', cursor: 'pointer',
          marginTop: 8,
        }}>閉じる</button>
      </div>
    </div>
...
export default function JournalingApp() {
    ...
    try {
      ...
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          messages: newHistory,
          step: currentStep,
          userInputs: feelings.filter(f => f.trim()),
        }),
      });
      const data = await res.json();
      if (data.error) throw new Error(data.error);

      const assistantMessage = JSON.stringify(data.result);
  ...
  const handleStepSubmit = async (currentStep: 2 | 3 | 4 | 5) => {
    ...
    if (currentStep === 5) {
      ...
      const result = await callAI(userMessage, nextStep);
      if (result) {
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 660, margin: '0 auto', padding: '0 20px', minHeight: '100vh' }}>
        ...
        <header style={{ textAlign: 'center', padding: '52px 0 36px' }}>
          <h1 style={{
            ...
            color: '#2c2520', letterSpacing: '0.14em',
            lineHeight: 1.5, fontWeight: 600,
          }}>
            私を好きになる<br />ジャーナリング
          </h1>
          <p style={{ fontSize: '0.75rem', color: '#8a7e79', letterSpacing: '0.2em', marginTop: 10 }}>
            — 自分の気持ちに、やさしく寄り添う —
          </p>
          <div style={{
            width: 60, height: 1,
          ...
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 8 }}>
              {[...Array(MAX_DAILY)].map((_, i) => (
                <div key={i} style={{
                  width: 10, height: 10, borderRadius: '50%',
                  background: i < usageCount ? '#d4847a' : '#e8d5b0',
                  transition: 'all 0.4s ease',
                }} />
              ))}
            </div>
            <p style={{ fontSize: '0.7rem', color: '#8a7e79', letterSpacing: '0.08em' }}>
              {remaining > 0
                ? `本日あと ${remaining} 回利用できます`
                : '本日の利用回数に達しました（毎朝0時リセット）'}
            </p>
          </div>
        )}
        ...
        {hasStarted && (
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 6, marginBottom: 28 }}>
            {([1, 2, 3, 4, 5, 6] as Step[]).map(s => (
              <div key={s} style={{
                width: s === step ? 28 : 9,
                height: 9, borderRadius: s === step ? 5 : '50%',
                background: s < step ? '#8ba888' : s === step ? '#c9a96e' : '#e8d5b0',
                transition: 'all 0.4s ease',
              }} />
            ))}
            <span style={{ fontSize: '0.72rem', color: '#8a7e79', letterSpacing: '0.08em', marginLeft: 8 }}>
              {STEP_LABELS[step]}
            </span>
          </div>
        )}
        ...
        {step === 1 && (
          <div style={{
            ...
            padding: '36px 36px',
            boxShadow: '0 4px 32px rgba(44,37,32,0.07)',
            ...
            <div style={{
              ...
              background: 'linear-gradient(90deg, #d4847a, #c9a96e, #8ba888)',
              opacity: 0.6,
            }} />

            <p style={{ fontSize: '0.95rem', lineHeight: 1.9, color: '#5a4f4a', marginBottom: 28 }}>
              今日の気持ち、出来事、もやもやしていること…<br />
              思いつくままに、5つ書いてみてください。
            </p>

            <div style={{ fontSize: '0.68rem', letterSpacing: '0.25em', color: '#8a7e79', marginBottom: 16, display: 'flex', alignItems: 'center', gap: 8 }}>
              今日の気持ち・出来事
                ...
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <input
                    ...
                    style={{
                      ...
                      color: '#2c2520', transition: 'all 0.3s ease',
                    }}
                    onFocus={e => {
            ...
            <button
              ...
              style={{
                width: '100%',
                background: feelings.filter(f => f.trim()).length >= 3 && remaining > 0
                  ? 'linear-gradient(135deg, #d4847a, #b85c52)'
                  : '#e8d5b0',
                color: 'white', border: 'none', borderRadius: 16,
                padding: '16px 32px',
                fontFamily: "'Zen Maru Gothic', sans-serif",
                fontSize: '0.95rem', letterSpacing: '0.08em',
                cursor: feelings.filter(f => f.trim()).length >= 3 && remaining > 0 ? 'pointer' : 'not-allowed',
                transition: 'all 0.3s ease',
                boxShadow: feelings.filter(f => f.trim()).length >= 3 && remaining > 0
                  ? '0 4px 20px rgba(184,92,82,0.3)' : 'none',
              }}
            >
              {remaining <= 0 ? '本日の利用回数に達しました' : '気持ちを送る →'}
            </button>

            {remaining <= 0 && (
              <p style={{ textAlign: 'center', marginTop: 12, fontSize: '0.8rem', color: '#8a7e79' }}>
                もっと話したい方は{' '}
            ...
            <div key={s} style={{
              ...
              {ai.deepDive && (
                <div style={{
                  ...
                  borderRadius: 14, borderLeft: '3px solid #c9a96e',
                }}>
                  <p style={{ fontSize: '0.875rem', lineHeight: 1.8, color: '#5a4f4a' }}>
                    {ai.deepDive}
                  </p>
                </div>
              )}
              ...
              <div style={{ marginBottom: 20 }}>
                {ai.suggestions.map((sug, i) => (
                  <SuggestionBtn
                    key={i}
                    text={sug}
                    selected={data.selectedSuggestion === sug}
                    onClick={() => updateStepField(s, 'selectedSuggestion', sug)}
                  />
                ))}
              </div>

              {/* 自由記述 */}
              <p style={{ fontSize: '0.8rem', color: '#8a7e79', marginBottom: 10 }}>
                {ai.inviteText || '自由に書いてもOKです'}
              </p>
              <textarea
                value={data.freeText}
                onChange={e => updateStepField(s, 'freeText', e.target.value)}
                placeholder="自由に書いてみてください…"
                rows={3}
                style={{
                  width: '100%', background: '#faf7f2',
                  border: '1.5px solid #e8d5b0', borderRadius: 16,
                  padding: '16px 20px', fontSize: '0.9rem',
                  color: '#2c2520', resize: 'vertical', lineHeight: 1.7,
              ...
              {loading ? <LoadingDots /> : (
                <button
                  onClick={() => handleStepSubmit(s)}
                  disabled={!data.selectedSuggestion && !data.freeText.trim()}
                  style={{
                    width: '100%',
                    background: (data.selectedSuggestion || data.freeText.trim())
                      ? 'linear-gradient(135deg, #d4847a, #b85c52)' : '#e8d5b0',
                    color: 'white', border: 'none', borderRadius: 16,
                    padding: '16px 32px',
                    fontFamily: "'Zen Maru Gothic', sans-serif",
                    fontSize: '0.95rem', letterSpacing: '0.08em',
                    cursor: (data.selectedSuggestion || data.freeText.trim()) ? 'pointer' : 'not-allowed',
                    transition: 'all 0.3s ease',
                    boxShadow: (data.selectedSuggestion || data.freeText.trim())
                      ? '0 4px 20px rgba(184,92,82,0.3)' : 'none',
                  }}
                >
                  {s === 5 ? 'まとめを見る ✨' : '次へ進む →'}
        ...
        {/* ローディング中（ステップ移行時） */}
        {loading && step !== 1 && !stepData[step]?.aiResponse && (
          <div style={{
            background: '#fffef9', borderRadius: 24,
            padding: '40px 36px', textAlign: 'center',
            boxShadow: '0 4px 32px rgba(44,37,32,0.07)',
            marginBottom: 20, animation: 'cardIn 0.5s ease forwards',
          }}>
            <p style={{ fontSize: '0.85rem', color: '#8a7e79', marginBottom: 16 }}>
              気持ちに寄り添っています…
            </p>
            <LoadingDots />
          </div>
        ...
        {step === 6 && summary && (
          <div style={{ animation: 'cardIn 0.6s ease forwards' }}>
            <div style={{
              background: 'linear-gradient(135deg, #fffbf5 0%, #faf7ff 100%)',
              ...
              <h2 style={{
                fontFamily: "'Shippori Mincho', serif",
                fontSize: '1.1rem', color: '#2c2520',
                letterSpacing: '0.12em', marginBottom: 28,
                textAlign: 'center',
              }}>✨ 本日のまとめ</h2>

              {[
                { label: '今日の出来事・事実', text: summary.facts, emoji: '📝' },
                { label: '最初の気持ち', text: summary.firstEmotion, emoji: '💭' },
                { label: '感情の変化', text: summary.emotionChange, emoji: '🌱' },
              ].map((item, i) => (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    color: '#c9a96e', marginBottom: 8, fontWeight: 500,
                  }}>
                    {item.emoji} {item.label}
                  </div>
                  <p style={{ fontSize: '0.93rem', lineHeight: 1.9, color: '#2c2520' }}>
                    {item.text}
                  </p>
                </div>
              ))}
              ...
              <div>
                <div style={{
                  fontSize: '0.65rem', letterSpacing: '0.28em',
                  color: '#d4847a', marginBottom: 12, fontWeight: 500,
                }}>
                  🌸 あなたへのメッセージ
                </div>
                <p style={{
                  fontFamily: "'Shippori Mincho', serif",
                  fontSize: '1rem', lineHeight: 2.1,
                  color: '#b85c52', whiteSpace: 'pre-wrap',
                }}>
            ...
            <button
              ...
              style={{
                ...
                fontFamily: "'Zen Maru Gothic', sans-serif",
                fontSize: '0.875rem', color: '#8a7e79',
                cursor: 'pointer', marginBottom: 40,
                transition: 'all 0.25s ease',
              }}
              onMouseEnter={e => {
                (e.target as HTMLButtonElement).style.borderColor = '#c9a96e';
                (e.target as HTMLButtonElement).style.color = '#5a4f4a';
              }}
              onMouseLeave={e => {
                (e.target as HTMLButtonElement).style.borderColor = '#e8d5b0';
                (e.target as HTMLButtonElement).style.color = '#8a7e79';
              }}
            >
              もう一度はじめる
            </button>
        ...
        <footer style={{ textAlign: 'center', padding: '20px 0 40px' }}>
          <p style={{ fontSize: '0.68rem', color: '#8a7e79', letterSpacing: '0.1em' }}>
            Studio Mizuki × AI Journaling
          </p>
        </footer>
      </div>
    </>
